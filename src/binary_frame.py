from bitstring import BitArray as bitstring_BitArray
...
class BinaryFrame:
    ...
    def __init__(self, data, start, end, years_per_block):
        ...
        data: a pandas DataFrame to convert
        """
        self.data = data
        self.bin_data = {}
        self.time = end - start
        self.years_per_block = years_per_block
        self.time_periods = math_floor(self.time / years_per_block)
        self.time_periods_fwd = math_floor(self.time - years_per_block + 1)
    ...
    def convert(self, method, convert=True, independent_samples=True):
        """
        A method for discretizing a pandas DataFrame
        into a Dictionary of Binary Strings.
        - If the return is +, then set the equivalent bit to 1
        - If the return is -, then set the equivalent bit to 0

        *Note that using this method compresses the data significantly
        return: none
        """
        for data_set in self.data.columns:    # For each data set i.e. security return sequences
            binary_streams = []
            day = 0
            days = len(self.data[data_set])
            days_in_stream = math_floor(days / self.time_periods)
            days_in_year = math_floor(days / self.time)
            ...
            while len(binary_streams) < samples:
                ...
                for j in range(day, (day + days_in_stream)):
                    ...
                    if method == "discretize":
                        ...
                        exit(0)
                    binary_stream += bit       # Add day to binary stream
                    if independent_samples:    # Keep track of days
                        day += 1
                if not independent_samples:
                    day += days_in_year
    ...
    def convert_basis_point(self, floating_point, convert=True):
        """
        This method converts a floating point number to an
        integer (basis points) and then converts it to binary.
        floating_point: floating point number
        convert: if true, then the number is not already an integer
    ...
    def convert_floating_point(self, floating_point, length=64):
        """
        This method converts a floating point number
        into a binary string using the IEEE 754 method.
        floating_point: floating point number
        length: the length of the resulting bit string
        return: a binary string
        """
        bin_r = bitstring_BitArray(float=floating_point, length=length)
    ...
    def discretize(self, floating_point):
        """
        This method discretizes the floating point
        number according to whether it is + or -
        floating_point: the floating point number to convert
        return: a binary string
        """
        if floating_point > 0.0:
            return '1'
    ...
    def flip_bits(self, bin_data):
        ...
        bin_data: the binary string
        return: flipped binaray string
        """
